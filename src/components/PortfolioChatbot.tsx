  ...
  return (
    <div className="fixed bottom-5 right-5 z-[60]">
        ...
        <div className="mb-3 flex h-[min(600px,calc(100vh-110px))] w-[min(380px,calc(100vw-32px))] flex-col overflow-hidden rounded-2xl border border-[#E7E1D8] bg-white shadow-[0_20px_60px_-20px_rgba(17,17,17,0.3)]">
          <div className="flex items-center justify-between bg-[#111111] px-4 py-3 text-white">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#C76B32]">
                <Bot size={21} />
              </div>
              <div>
                <p className="font-semibold">Rudra</p>
                <p className="text-xs text-white/60">Ajeet's Portfolio Assistant</p>
              </div>
            </div>
            <button
              onClick={() => setOpen(false)}
              aria-label="Close Rudra"
          ...
          <div className="flex-1 space-y-3 overflow-y-auto bg-[#FAF8F3] p-4">
              ...
              <div className="space-y-2 pt-1">
                {starterQuestions.map((question) => (
                  <button
                    ...
                    className="block w-full rounded-xl border border-[#E7E1D8] bg-white px-3 py-2 text-left text-xs text-[#444] transition hover:border-[#E5A06A] hover:bg-[#FFF9F4]"
                  >
                    {question}
                  </button>
                ))}
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="rounded-2xl rounded-bl-md border border-[#E7E1D8] bg-white px-4 py-3 text-sm text-[#777]">
                  Rudra is thinking<span className="animate-pulse">...</span>
                </div>
              </div>
            )}
          ...
          <form
            ...
            className="border-t border-[#E7E1D8] bg-white p-3"
          >
            <div className="flex items-center gap-2 rounded-xl border border-[#E7E1D8] bg-[#FAF8F3] px-3 py-1.5 focus-within:border-[#C76B32]">
              ...
              <button
                ...
                className="rounded-lg bg-[#C76B32] p-2 text-white transition hover:bg-[#ad5826] disabled:cursor-not-allowed disabled:opacity-40"
              >
                <Send size={16} />
              </button>
            </div>
      ...
      <button
        ...
        aria-label={open ? 'Close Rudra chatbot' : 'Open Rudra chatbot'}
        className="group flex items-center gap-2 rounded-full border border-[#E7E1D8] bg-white p-2 pr-4 shadow-[0_12px_35px_-12px_rgba(17,17,17,0.35)] transition duration-300 hover:-translate-y-1 hover:border-[#E5A06A]"
      >
        <span className="relative flex h-12 w-12 items-center justify-center rounded-full bg-[#C76B32] text-white">
          ...
          {!open && (
            <span className="absolute right-0 top-0 h-3 w-3 rounded-full border-2 border-white bg-[#55A630]" />
          )}
        </span>
